...
class FetchPerformanceSummaryBehaviour(
            ...
            if self.params.is_running_on_polymarket:
                ...
                for participant in stat.get("profitParticipants", []):
                    ...
                    if title:
                        titles.add(title)
            for title in titles:
                if title:  # pragma: no cover  # defensive: titles set already filters empty strings
    ...
    def _perform_incremental_update(
        ...
        if replace_last and new_data_points:
            ...
            if last_dp_day in incoming_days:  # pragma: no cover  # defensive: replace_last guarantees membership
                new_data_points.pop()
