...
from packages.valory.skills.abstract_round_abci.tests.test_models import BASE_DUMMY_SPECS_CONFIG, BASE_DUMMY_PARAMS
from packages.valory.skills.market_manager_abci.models import SharedState, Subgraph, OmenSubgraph, NetworkSubgraph, MarketManagerParams
...
MARKET_MANAGER_PARAMS = dict(creator_per_subgraph=dict(creator_per_subgraph=[]),
                             slot_count=2,
                             opening_margin= 1,
                             languages=[],
...
class TestSubgraph:
    ...
    @pytest.mark.parametrize(
        ...
        (
                ...
                #                  b'"error": {"key": {"the_graph_error_message_key": "the_graph_payment_required_error"}'
                #         ),
                #         None,
                #         None,
                # ),
...
class TestMarketManagerParams(unittest.TestCase):
    """Test MarketManagerParams of MarketManager."""
    ...
    def test_initialization(self,
        ...
        """Test initialization"""
        MarketManagerParams(test_case)

    def test_creators_iterator(self) -> None:
        """Test creators iterator."""
        market_manager_params=MarketManagerParams(**BASE_DUMMY_PARAMS,
                                                  **MARKET_MANAGER_PARAMS)

        creators_iterator = market_manager_params.creators_iterator
        self.assertIsInstance(creators_iterator, Iterator)
        for i in creators_iterator:
            print(i)