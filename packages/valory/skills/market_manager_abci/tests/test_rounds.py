...
def get_payloads(
    ...
    """Get payloads."""
    if data is None:
        return {}  # Return empty dictionary if data is None
    return {
...
class BaseMarketManagerRoundTestClass(BaseCollectSameUntilThresholdRoundTest):
    ...
    def run_test(self, test_case: RoundTestCase, **kwargs: Any) -> None:
        ...
        # Add debug print to understand the input data
...
class TestUpdateBetsRound(BaseMarketManagerRoundTestClass):
        ...
        (
            RoundTestCase(
                ...
                synchronized_data_attr_checks=[
                    ...
                    lambda synchronized_data: synchronized_data.participant_to_bets == DUMMY_PARTICIPANT_TO_BETS,
                ],
            ),
            RoundTestCase(
                name="Fetch error",
    ...
    def test_run(self, test_case: RoundTestCase) -> None:
        ...
        print(f"Initial Data: {test_case.initial_data}")
        print(f"Payloads: {test_case.payloads}")
        print(f"Expected Final Data: {test_case.final_data}")

        # Run the actual test
        self.run_test(test_case)
        
        # Debug output after running the test
        print(f"Debugging After Test:")
        for behaviour_attr_getter in test_case.synchronized_data_attr_checks:
            actual_value = behaviour_attr_getter(self.synchronized_data)
            expected_value = behaviour_attr_getter(
                deepcopy(self.synchronized_data)
            )
            print(f"Attribute Getter: {behaviour_attr_getter}")
