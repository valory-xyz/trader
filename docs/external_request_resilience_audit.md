...
| 6 | **HIGH** | `connections/polymarket_client/connection.py` | `_fetch_markets` can block worker thread for 11+ minutes (BP8) |
| 16 | **HIGH** | `decision_maker_abci/handlers.py:253` | No global try-catch in handler dispatch |
| 17 | **HIGH** | `trader_abci/handlers.py:279` | `_handle_get_agent_info` has no error handling, crashes pre-FSM |
| 25 | **HIGH** | `predictions_helper.py:526` | Wrong bet returned: `next(..., bets[0])` falls back to first bet instead of `None` |
| 3 | **MEDIUM** | `market_manager_abci/graph_tooling/requests.py:244,363` | Batched trade fetching crashes on missing dict keys |
| 4 | **MEDIUM** | `market_manager_abci/graph_tooling/requests.py` | Retry backoff (31s) exceeds round timeout (30s) |
| 10 | **MEDIUM** | `trader_abci/handlers.py:117` | Stale hardcoded `FALLBACK_POL_TO_USD_RATE` (30 days old) |
| 12 | **MEDIUM** | `trader_abci/handlers.py:502` | Synchronous `requests.get` in handler blocks event loop up to 10s |
| 14 | **MEDIUM** | `trader_abci/handlers.py:669` | Web3 HTTPProvider has no timeout configured |
| 15 | **MEDIUM** | `trader_abci/handlers.py:855` | `_estimate_gas` returns `False` instead of `None` (type bug) |
| 18 | **MEDIUM** | `chatui_abci/handlers.py:202` | `json.loads` without try-except crashes on malformed POST |
| 19 | **MEDIUM** | `trader_abci/handlers.py:593` | `_handle_get_funds_status` has no try-except |
| 21 | **MEDIUM** | `trader_abci/handlers.py:599` | No deduplication of x402 swap tasks -- duplicate transactions possible |
| 7 | **MEDIUM** | `connections/polymarket_client/connection.py` | No retries on `_fetch_market_by_slug`, `_get_positions`, `_get_trades` |
| 23 | **MEDIUM** | `agent_performance_summary_abci/graph_tooling/requests.py:434` | `UnicodeDecodeError` not caught in OLAS price fetch |
| 24 | **MEDIUM** | `predictions_helper.py:152` | `{"data": null}` -> `AttributeError` -- missing `or {}` guard |
| 26 | **MEDIUM** | `polymarket_predictions_helper.py:136` | `{"data": null}` -> `AttributeError` -- missing `or {}` guard |
| 27 | **MEDIUM** | `agent_performance_summary_abci/behaviours.py:1341` | Uncaught `KeyError` on missing `"date"` in daily profit statistics |
| 11 | **LOW** | `trader_abci/handlers.py:516` | `if not price_usd` treats zero as missing (BP4) |
| 13 | **LOW** | `decision_maker_abci/behaviours/storage_manager.py` | `_fetch_accuracy_info` has no max retry bound |
| 20 | **LOW** | `trader_abci/handlers.py:328` | `_handle_get_static_file` only catches `FileNotFoundError` |
| 22 | **LOW** | `connections/polymarket_client/connection.py:253` | `json.loads` outside try-except in `on_send` |
...
| **P1** | BUG 21: No deduplication of x402 swap tasks | Side-effect | Low | Add a `self._swap_in_progress` flag checked before `executor.submit()`. Use `threading.Lock` or check `Future.running()`. |
| **P1** | BUG 10: Stale fallback exchange rate | Side-effect | Medium | Add timestamp tracking; refuse to use fallback older than N hours; log critical alert. |
| **P1** | BUG 25: Wrong bet returned in position details | Side-effect | Low | Change `next(..., bets[0])` to `next(..., None)` at `predictions_helper.py:526`. |
| **P2** | BUG 2: fetch_claim_params KeyError risk | Crash | Medium | Wrap the list comprehension at `requests.py:298-313` in try/except KeyError, or use `.get()` with validation. |
...
| **P2** | BUG 27: KeyError on missing "date" in profit stats | Crash | Low | `stat.get("date")` with fallback, or wrap loop body in try-except. |
| **P3** | BUG 23: UnicodeDecodeError in OLAS price fetch | Crash | Low | Wrap `response.body.decode()` in try-except at `requests.py:434`. |
| **P3** | BUG 6: _fetch_markets blocks thread 11+ min | Stuck | Medium | Cap total retry time per `_fetch_markets` call. Add a deadline parameter. Consider parallelizing category fetches. |
| **P3** | BUG 4: Retry backoff exceeds round timeout | Stuck | Low | Cap cumulative sleep to 80% of round timeout, or make retry logic aware of remaining time. |
| **P3** | BUG 19: _handle_get_funds_status no try-except | Crash | Low | Wrap body in try/except Exception. |
| **P3** | BUG 3: Batched trade fetching KeyError | Crash | Low | Use `.get()` with validation for `fpmm` and `creationTimestamp` keys. |
| **P3** | C3: Category concentration risk | Side-effect | High | Add minimum-category-count check before proceeding with trading. |
| **P3** | CC1: Inconsistent retry strategies | All | High | Standardize retry configuration. Create a shared retry utility with configurable max retries, backoff type, and timeout awareness. |
| **P3** | CC2: No circuit breaker | All | High | Implement circuit breaker pattern for repeated failures to the same service. Track consecutive failures; skip retries for N seconds after M failures. |
| **P3** | B9: Reset loop under sustained outage | Stuck | Medium | Add exponential backoff between periods during repeated failures. |
| **P4** | BUG 7: Inconsistent retries in connection | Various | Medium | Add retry logic to `_fetch_market_by_slug`, `_get_positions`, `_get_trades`. |
