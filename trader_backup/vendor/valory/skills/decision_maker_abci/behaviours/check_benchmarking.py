...
"""This module contains the behaviour for checking whether the benchmarking mode is enabled."""
...
class CheckBenchmarkingModeBehaviour(DecisionMakerBaseBehaviour):
    """A behaviour in which the agents blacklist the sampled bet."""

    matching_round = CheckBenchmarkingModeRound

    def async_act(self) -> Generator:
        """Do the action."""
        with self.context.benchmark_tool.measure(self.behaviour_id).local():
            payload = VotingPayload(
                self.context.agent_address, self.benchmarking_mode.enabled
            )
